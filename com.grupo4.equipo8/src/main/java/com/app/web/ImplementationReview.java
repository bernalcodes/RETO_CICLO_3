...
public class ImplementationReview {
	public static void main(String[] args) {
		 ...
		 * then.format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss")));
		 * 
		 ...
		 * int monthFee = 220000;
		 * 
		 * System.out.println("\nnow.until(then, ChronoUnit.MINUTES): " +
		 * minutesElapsed);
		 * System.out.println("now.until(then, ChronoUnit.DAYS): " + daysElapsed *
		 * 1440);
		 * 
		 * int payment = 0;
		 ...
		 * }
		 * 
		 * payment = (monthsElapsed * monthFee) + (daysElapsedTest * dayFee) +
		 * (minutesElapsed * minuteFee);
		 * 
		 ...
		 * 
		 * System.out.println("\nHa estado registrado por: " + monthsElapsed +
		 * " meses, "
		 * + daysElapsed + " días y " + minutesElapsed + " minutos");
		 * 
		 * String returnPaymentString = NumberFormat.getCurrencyInstance(new
		 * Locale("en", "US")).format(payment);
		 * 
		 * System.out.
		 * println("\nEl valor a pagar por su estadía en el parqueadero es de: " +
		 * returnPaymentString);
		 */

		BCryptPasswordEncoder encoder = new BCryptPasswordEncoder();
		System.out.println(encoder.encode("toor"));
	}
}
